...
while (!choosedLayout.isInteger() || ((choosedLayout as Integer) > 3 && (choosedLayout as Integer) < 1)) {
    choosedLayout = showInputDialog('Wybierz leyout\nWpisując 1 wybierasz FlowLayout\nWpisując 2 wybierasz BorderLayout\nWpisując 3 wybierasz GridLayout 3x2')
}

def swing = new SwingBuilder().edt {
    frame(title: 'Buttons', defaultCloseOperation: JFrame.EXIT_ON_CLOSE, pack: true, show: true, layout: lm[(choosedLayout as Integer) - 1]) {
